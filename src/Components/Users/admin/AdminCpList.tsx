...
export default function AdminCpList() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          "https://ipaisa.co.in/api/auth/admincps",
          {
            headers: headers,
          }
        );
  ...
  const handleDelete = async (itemId: any) => {
    try {
      ...
      await axios.delete(
        // `https://ipaisa.co.in/api/delete/Channel_partner/${itemId}`,
        `https://ipaisa.co.in/api/protected/delete/${itemId}`,
        {
          headers: headers,
        }
      );
  ...
  const handleEdit = async (itemId: any) => {
    try {
      ...
      await axios.put(
        `http://api.ipaisa.site/api/update/channel_Partner/${itemId}`,
        {
          headers: headers,
        }
      );
  ...
  return (
    <section className=" w-full flex flex-col !gap-6 my-8 h-[calc(100vh-9rem)]">
      <div className="w-full flex justify-start items-center gap-12">
        <div className=" h-32 ">
          <p className="items-start mb-5 font-semibold text-md">User name</p>
          <TextField
            type="text"
